...
使用项目配置的 `deepseek-v4-pro` 经 AgentRunner 自主选择工具，模型与工具均未模拟。
测试进程临时放行所测工具，没有修改持久化权限配置；浏览器表单使用本地测试服务。

| 任务 | 实测结果 |
| --- | --- |
| 读取给定官方文档 | 8.25 秒完成，仅调用 `web_fetch`，正确返回标题、摘要与来源 |
| 填写并提交测试表单 | 13.96 秒完成导航、快照、输入、点击与再次快照；随机回执与服务端一致，提交次数为 1 |
| 搜索并阅读来源 | 两次搜索均返回空结果，模型随后读取已知官方 URL；严格的搜索发现来源 → 阅读来源检查未通过 |

直接补测三组搜索，两组返回结果、一组为空。这些少量样本只能说明当前存在不稳定表现，
不能据此估计整体成功率，也不能把模型最终回答成功当作搜索链路通过。
安装版 DDGS 会将非 200 HTTP 响应丢弃，最终可能表现为 `No results found.`；
当前适配层无法据此区分真正无匹配与上游错误，本次空返回的具体原因尚未确定。

独立复跑 94 项相关测试全部通过，mypy 检查 101 个源码文件无问题。
真实浏览器的正常关闭、动作执行中取消、关闭任务自身取消各测试 3 次，共 9 次全部通过；
累计跟踪的 81 个进程均退出，对应进程组和临时目录均清理完成。
