...
| 入口 | 实际行为 |
| --- | --- |
| “工作区”标题旁的 +、⌘ O | 打开系统文件夹选择器，切换到该目录的独立 Core |
| 工作区行旁的 + | 在该工作区新建对话；顶部“新对话”使用当前工作区 |
| 工作区行的折叠箭头 | 展开或收起该工作区的聊天记录，不改变当前执行目录 |
| 输入框上方工作区名 | 紧凑浮层搜索、切换项目或默认工作区；悬停查看实际目录 |
...
附件发送后存入 Core 历史；尚未发送的附件仅留在当前窗口内存，切换工作区或重启后会列出需重新添加的文件名。
...
侧栏按“工作区 → 对话”组织，不再将项目与全局最近记录分成两列无关联的列表。
默认工作区固定在最上方；各项目内先显示置顶对话，其余按最近更新时间排列。
搜索会展开各组，并在原工作区下显示匹配标题和已缓存正文的结果；清除搜索后恢复折叠状态。
点击其他项目的历史记录时，先切换实际目录与事件连接，再恢复目标对话及其草稿。
未发送的文字和附件也以草稿显示在所属工作区；有草稿时新建对话会保留原稿并打开空白输入。
从插件等页面点击当前工作区会回到聊天。后端已不存在的会话会移出列表，其中未发送的内容保留为可继续发送的本地草稿。
展开未启动项目只读取会话元数据；已运行项目从对应 Core 获取摘要，不因浏览历史启动模型或插件。

默认工作区使用 `~/.mini/desktop/workspace`，首次启动自动创建，不要求 Git 仓库或项目配置。
指定 `--storage-path` 时，目录位于该存储路径下的 `workspace/`。
它支持普通聊天、文件操作、附件与会话历史，使用独立 Core，且不能从列表移除。
首次启动仍可从当前项目开始；旧版本保存的空项目选择会迁移到默认工作区。
断线不会自动重发消息，避免重复执行任务。
应用管理的 Core 意外退出后，重连会重新启动并恢复会话；缺少模型密钥会返回配置错误，保持 Core 和项目导航可用。

切换项目时保留旧项目的 Core，因此后台任务不会被切换操作结束。
移除当前项目会回到默认工作区；移除最后一个项目后仍可继续聊天，重启保持默认选择。
...
工作区功能修复通过 `tests/unit/test_desktop_runtime.py` 中真实 Core 子进程和网关测试，
覆盖缺密钥首次发送、Core 退出后的重连与历史恢复；使用临时存储和隔离环境，不读取用户密钥。
`workspace-drafts.e2e.js` 补充多份草稿、当前项目导航、跨项目恢复、失效会话和连续切换检查，
与 `workspace-sidebar.e2e.js` 一起覆盖分组历史的实际交互。
同时在实际 macOS 窗口验收两份中文草稿、新建空白对话、管理页返回、跨项目直接打开草稿和退出重启恢复。
默认工作区使用真实模型完成重启后续聊，并再次执行 `bash pwd`，确认工具目录和会话历史均正确；自测会话已清理。
...
`projects.e2e.js` 覆盖 13 个选择、移除、默认工作区和运行保护检查点，
`projects.reconnect.e2e.js` 覆盖 5 个双窗口断线恢复检查点。
`workspace-sidebar.e2e.js` 覆盖 13 个分组归属、置顶、跨项目历史和草稿、搜索、折叠持久化、默认目录与窄窗口检查点。
这三个项目测试文件需分别从新启动的夹具运行，避免共享已移除的测试项目列表。
夹具为项目和默认工作区分配独立 TCP 端口与事件总线，验证历史与事件不会串到其他目录。
