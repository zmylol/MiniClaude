...
export class WorkspaceViews {
  ...
  settingsPage() {
    ...
    return `<div class="settings-section"><h3>工作区</h3><div class="setting-row"><div><strong>${esc(info.project_name)}</strong><p>${esc(info.project_path)}</p></div>${button('pick-project', '打开其他项目')}</div><div class="setting-row"><div><strong>模型</strong><p>${esc(this.active()?.selectedModel || info.model)}</p></div>${button('model', '选择模型')}</div><div class="setting-row"><div><strong>工具权限</strong><p>只读、按需审批或完全访问；每个会话单独设置。</p></div>${button('permissions', '设置权限')}</div></div><div class="settings-section"><h3>快捷键</h3><div class="setting-row"><span>新对话</span><kbd>⌘ ⇧ O</kbd></div><div class="setting-row"><span>搜索所有对话</span><kbd>⌘ K</kbd></div><div class="setting-row"><span>展开或收起侧栏</span><kbd>⌘ B</kbd></div><div class="setting-row"><span>添加文件</span><kbd>⌘ ⇧ A</kbd></div><div class="setting-row"><span>打开项目文件夹</span><kbd>⌘ O</kbd></div><div class="setting-row"><span>设置</span><kbd>⌘ ,</kbd></div></div><div class="settings-section"><h3>本机数据</h3><p>对话保存在本机 Core 中，重启应用后可继续。草稿和界面偏好保存在此桌面工作区。</p></div>`;
  }
