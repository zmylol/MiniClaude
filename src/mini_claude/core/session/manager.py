...
class SessionManager:
    ...
    def __init__(
        ...
        self._running_runs: dict[str, tuple[asyncio.Task[Any], str]] = {}
    ...
    async def send_message(self, sid: str, content: str, *, run_id: str | None = None) -> str:
        ...
        async with lock:
            ...
            self._running_runs[run_id] = (run_task, sid)
            try:
                await run_task
    ...
    # 请求取消仍在执行的 run；找不到或已结束时返回 False
    def cancel_run(self, run_id: str) -> bool:
        active = self._running_runs.get(run_id)
        if active is None:
            return False
        task, sid = active
        if task.done():
        ...
        if self._permission_manager is not None:
            self._permission_manager.cancel_session(sid, reason="run_cancelled")
        return task.cancel()
